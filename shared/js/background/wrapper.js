...
export class BrowserWrapper {
    // Session storage
    // @ts-ignore
    sessionStorageSupported = typeof browser.storage.session !== 'undefined'
    sessionStorageFallback = this.sessionStorageSupported ? null : new Map()
    ...
    openExtensionPage (path) {
        browser.tabs.create({ url: this.getExtensionURL(path) })
    }
     ...
     */
    async setActionIcon (iconPath, tabId) {
    ...
    // @ts-ignore
    async getFromStorage (key, cb) {
        const result = await browser.storage.local.get(key)
        return result[key]
    }

    // @ts-ignore
    async getFromManagedStorage (keys, cb) {
        try {
    ...
    mergeSavedSettings (settings, results) {
     ...
     */
    async setToSessionStorage (key, data) {
     ...
     * @return {Promise<undefined>}
     */
    async removeFromSessionStorage (key) {
        if (typeof key !== 'string') {
            throw new Error('Invalid storage key, string expected.')
        }
