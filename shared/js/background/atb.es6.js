...
/**
 * @typedef {Object} ATBType
 * @property {*} addParametersMainFrameRequestUrl
 * @property {*} setInitialVersions
 * @property {*} getAcceptedParamsFromURL
 * @property {*} finalizeATB
 * @property {*} canShowPostInstall
 * @property {*} getSurveyURL
 * @property {*} updateATBValues
 * @property {*} openPostInstallPage
 */
...
export const shouldUpdateSetAtb = (request) => {
    return matchPage.test(request.url)
}

export const updateSetAtb = async () => {
...
*   false otherwise.
*/
export const addParametersMainFrameRequestUrl = (url) => {
...
export const setInitialVersions = (numTries) => {
    numTries = numTries || 0
    ...
    return load.JSONfromExternalFile(url).then((res) => {
        ...
        return new Promise((resolve) => {
            ...
            return setInitialVersions(numTries)
        })
    })
}

const finalizeATB = (params) => {
...
export const getAcceptedParamsFromURL = (url) => {
...
export const openPostInstallPage = async () => {
...
export const canShowPostInstall = (domain) => {
...
settings.ready().then(async () => {
    ...
    browserWrapper.setUninstallURL(await getSurveyURL())
})
