          ...
          return new Promise ((resolve, reject) => {
              backgroundPage.utils.getCurrentTab((rawTab) => {
                  if (rawTab) {
                      ...
                      self.potentialBlocked = self.tab.trackers || {};
                      // self.trackersBlocked = self.tab.trackers || {};
                      const companyNames = Object.keys(self.potentialBlocked);

                      // find largest number of trackers (by company)
                      let maxCount = 0;
                      // if (self.trackersBlocked && companyNames.length > 0) {
                      if (self.potentialBlocked && companyNames.length > 0) {
                          companyNames.map((name) => {
                              ...
                              if (name !== 'unknown') {
                                  ...
                                  let compare = self.potentialBlocked[name].count;
                      ...
                      self.companyListMap = companyNames.map(
                          (companyName) => {
                              let company = self.potentialBlocked[companyName];
