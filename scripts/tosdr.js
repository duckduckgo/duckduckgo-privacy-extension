...
function getSites () {
    ...
    request.get('https://tosdr.org/index/services.json', (err, res, body) => {
        ...
        const sites = Object.keys(JSON.parse(body))
...
function getSitePoints (sites) {
    return new Promise((resolve, reject) => {
        ...
        const site = encodeURIComponent(sites.pop())
        nProcessed += 1

        const githubRepo = 'https://raw.githubusercontent.com/tosdr/tosdr.org/master'
        const url = `${githubRepo}/api/1/service/${site}.json`
        ...
        request.get(url, (err, res, body) => {
            ...
            const points = { score: 0, all: { bad: [], good: [] }, match: { bad: [], good: [] } }
            let allData
            ...
            const pointsData = allData.pointsData
            const relatedUrls = allData.urls || []

            points.class = allData.class

            for (const pointName in pointsData) {
                const point = pointsData[pointName]
                ...
                const type = point.tosdr.point

                if (type === 'good' || type === 'bad') { addPoint(points, type, pointCase, score) }
            }
            ...
            // get site url
            const servicesUrl = `${githubRepo}/services/${site}.json`
