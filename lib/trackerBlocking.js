...
// Tracker entry's rules are matched in order, to acheive that the corresponding
...
// ceiling prioirty from being exceeded.
...
// other aspects of the extension, set an arbritrary limit of 900 for
// Tracker Blocking.
...
function generateDNRRule ({
    priority, actionType, urlFilter, regexFilter, resourceTypes,
    requestDomains, initiatorDomains, excludedInitiatorDomains,
    matchCase = false
}) {
    const dnrRule = {
        priority,
...
function processTrackerRule (domain, trackerRule, matchCnames) {
    ...
    if (urlFilter.startsWith(domain)) {
        ...
        // If there isn't a (known) domain part, then case senstive matching can
...
function getTrackerEntryDomain (tds, domain, skipSubdomains = 0) {
    ...
    do {
        ...
        const trackerEntry = tds.trackers[domain]
        if (trackerEntry) {
            return domain
        }
    } while (i > -1)
...
function finalizeDNRRulesAndLookup (startingRuleId, dnrRules) {
    ...
    for (const rule of dnrRules) {
        ...
        if (ruleIdByByStringifiedDNRRule.has(key)) {
            ...
            //       storing a referece to the existing array. That won't work
            //       since the array will be mutated to add the request domains
            //       for any duplicate rules.
...
async function generateTrackerBlockingRuleset (
    ...
    for (const [domain, cname] of Object.entries(tds.cnames)) {
        ...
        // to.
        const trackerEntryDomain = getTrackerEntryDomain(tds, cname)
        if (trackerEntryDomain) {
            ...
            if (getTrackerEntryDomain(tds, domain, 1)) {
                continue
            }
